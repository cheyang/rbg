...
> **注意启动顺序**：如果 Router 先于 Prefill/Decode 启动，由于 `publishNotReadyAddresses: true`，DNS 可以解析但后端尚未就绪。Router 的连接尝试会被拒绝。大多数推理引擎的 Router（如 SGLang Router）内置了重试机制，会等待后端就绪后自动连接。如果你使用的 Router 不具备重试能力，建议通过 `dependencies` 字段声明 Router 对 Prefill/Decode 角色的依赖，确保后端角色先启动。
>
...
> **注意**：以下示例使用 Qwen3-0.6B 小模型仅作 YAML 语法演示。实际部署中，张量并行适用于单卡显存无法加载的大模型（如 70B+ 参数）。请替换为实际需要多 GPU 的模型。
>
...
# 查看 Pod 状态（使用 RBG 自动添加的标签）
kubectl get pods -l rbg.workloads.x-k8s.io/group-name=<rbg-name>

# 查看特定角色的 Pod
kubectl get pods -l rbg.workloads.x-k8s.io/group-name=<rbg-name>,rbg.workloads.x-k8s.io/role-name=<role-name>
...
### Q: leaderWorkerPattern 中 Worker Pod 启动后不断 CrashLoopBackOff？

这通常是因为 Worker 先于 Leader 启动，推理引擎尝试连接 Leader 的分布式初始化地址失败。这是**正常行为**——Kubernetes 会自动重启 Worker，直到 Leader 就绪后 Worker 即可成功连接。如需避免这种情况，可以在角色的 Pod 模板中配置 `initContainers`，等待 Leader 就绪后再启动。

### Q: PD 分离部署中 Router 启动报连接拒绝？

Router 配置的 Prefill/Decode DNS 地址虽然可以解析（`publishNotReadyAddresses: true`），但后端 Pod 可能尚未就绪。检查 Prefill/Decode Pod 是否已进入 Running 状态。如果 Router 不支持自动重试，可以通过 `dependencies` 字段声明依赖关系：
...
### Q: 如何确认 `tp-size` 与 `leaderWorkerPattern.size` 是否匹配？

两者必须一致。如果 `tp-size=4` 但 `size=2`，引擎会因为分布式初始化参与者不足而挂起。检查方式：
```bash
kubectl get pods -l rbg.workloads.x-k8s.io/group-name=<rbg-name> | wc -l
```
Pod 数量应等于 `replicas × size`。

### Q: 如何查看 RBG 创建的 Headless Service？
...
+ [Gang 调度配置](../features/gang-scheduling.md)
+ [弹性伸缩](../features/autoscaler.md)
