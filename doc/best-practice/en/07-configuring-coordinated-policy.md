...
| `spec.policies[].strategy.scaling` | object | No | Coordinated scaling strategy (only affects initial deployment) |
| `spec.policies[].strategy.rollingUpdate` | object | No | Coordinated upgrade strategy |

> **Note**: `scaling` and `rollingUpdate` can be configured together or used independently. `scaling` only takes effect during the initial deployment phase and does not affect subsequent autoscaling behavior after deployment completes. A single CoordinatedPolicy can contain multiple rules, each applying to different role groups.
>
...
> **Note**: Coordinated scaling only affects the replica creation rate during the initial deployment phase and does not affect runtime autoscaling behavior. Once initial deployment completes, this strategy no longer takes effect.
>
...
| --- | --- | --- | --- | --- |
| `strategy.scaling.maxSkew` | intOrString | No | `"100%"` | Allowed deployment progress deviation between roles. Can be an absolute value (e.g., `2`) or a percentage (e.g., `"10%"`) |
| `strategy.scaling.progression` | string | No | `OrderScheduled` | Creation pace: `OrderScheduled` or `OrderReady` |
...
| Batch | Prefill Progress | Decode Progress | Progress Difference | Within maxSkew |
| --- | --- | --- | --- | --- |
| 1 | `0 → 1` (20%) | `0 → 1` (10%) | 10% | ≤ 10%, allowed to continue |
| 2 | `1 → 2` (40%) | `1 → 3` (30%) | 10% | ≤ 10%, allowed to continue |

And so on — the two roles always maintain proportional progressive creation. If Prefill creates too fast and the progress difference exceeds 10%, the Controller pauses Prefill creation and waits for Decode to catch up.
...
| `strategy.rollingUpdate.maxUnavailable` | intOrString | No | - | Maximum proportion of unavailable instances across all roles during coordinated update |
| `strategy.rollingUpdate.partition` | intOrString | No | - | Global partition value for coordinated update (shared by all roles) |
...
3. **Coordinate update order**: If Prefill updates too fast, the Controller pauses Prefill's update and waits for Decode to catch up
...
Progress difference: `57% - 33% = 24%`, exceeds `10%`. The Controller pauses Prefill's update and prioritizes Decode's update.
...
<!-- TODO: The following documents have not been created yet. Links will be added after documentation is complete. -->
