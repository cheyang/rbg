...
var _ = Describe("PR418 F1 StatefulSet leaderWorkerPattern", func() {
	...
	It("Should accept an unset policy, reject an explicit LeaderOnly, and still narrow the "+
		...
		// [CONTRACT] -- guards the fix from 926778e7. The rest of this spec still carries weight
		// independently: a real API server refuses the explicit value and accepts the unset one, so
		// it pins down that admission alone never protected this path.
		Expect(svc.Spec.Selector).ShouldNot(HaveKey(constants.ComponentNameLabelKey),
			"F1: the controller must not narrow the shared service to component-name=leader on a "+
				"StatefulSet role -- no pod of that workload carries the label, and it is the exact "+
				"policy the API server refuses to let the user set explicitly")
	})
})
