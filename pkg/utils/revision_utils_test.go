...
func TestNewRevision(t *testing.T) {
	...
	revision1, err := NewRevision(ctx, client, rbg)
	assert.NoError(t, err)
	assert.NotNil(t, revision1)

	for i := 0; i < 10; i++ {
		revision2, err := NewRevision(ctx, client, rbg)
		assert.NoError(t, err)
...
func TestGetRolesRevisionHash(t *testing.T) {
	...
	t.Run("ConsistentHashForSameRoleContent", func(t *testing.T) {
		...
		revisionDiff, err := NewRevision(ctx, client, rbgDiff)
		assert.NoError(t, err)
		...
		for i := 0; i < 10; i++ {
			revision2, err2 := NewRevision(ctx, client, rbg)
